...
const TAG: &str = "Main";
const REQUIRED_APK_VERSION_CODE: &str = "1";
const CLIENT_PACKAGE: &str = "io.github.innocent254.revbridge";
const CLIENT_COMPONENT: &str =
    "io.github.innocent254.revbridge/io.github.innocent254.revbridge.GnirehtetActivity";
const CLIENT_START_ACTION: &str = "io.github.innocent254.revbridge.START";
const CLIENT_STOP_ACTION: &str = "io.github.innocent254.revbridge.STOP";
const REVERSE_SOCKET: &str = "localabstract:revbridge";
...
fn get_apk_path() -> String {
    if let Some(env_apk) = std::env::var_os("REVBRIDGE_APK") {
        env_apk.into_string().expect("invalid REVBRIDGE_APK value")
    } else if let Some(legacy_apk) = std::env::var_os("GNIREHTET_APK") {
        legacy_apk
            .into_string()
            .expect("invalid GNIREHTET_APK value")
    } else {
        "revbridge-client.apk".to_string()
    }
}
...
impl Command for TunnelCommand {
    ...
    fn description(&self) -> &'static str {
        "Set up the 'adb reverse' tunnel.\n\
         If a device is unplugged then plugged back while RevBridge is\n\
...
fn cmd_install(serial: Option<&str>) -> Result<(), CommandExecutionError> {
    info!(target: TAG, "Installing RevBridge client...");
    exec_adb(serial, vec!["install".into(), "-r".into(), get_apk_path()])
}

fn cmd_uninstall(serial: Option<&str>) -> Result<(), CommandExecutionError> {
    info!(target: TAG, "Uninstalling RevBridge client...");
    exec_adb(serial, vec!["uninstall", CLIENT_PACKAGE])
}
...
fn print_usage() {
    let mut msg = "Syntax: revbridge-relay (".to_string();
        ...
        match command {
            ...
            None => {
                if command_name == "rt" {
                    error!(
                        target: TAG,
                        "The 'rt' command has been renamed to 'run'. Try 'revbridge-relay run' instead."
                    );
