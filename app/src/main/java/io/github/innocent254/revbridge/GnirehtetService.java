...
public class GnirehtetService extends VpnService {
    ...
    private static final String ACTION_START_VPN = "io.github.innocent254.revbridge.START_VPN";
    private static final String ACTION_CLOSE_VPN = "io.github.innocent254.revbridge.CLOSE_VPN";
    ...
    public static void stop(Context context) {
        // This request always comes from the visible control activity or the
        // foreground notification action, so a regular service start is allowed.
        // Avoid starting a brand-new foreground service only to stop it.
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (ACTION_START_VPN.equals(action)) {
            if (isRunning()) {
                ...
                VpnConfiguration config = getVpnConfiguration(intent);
                if (config == null) {
    ...
    private static VpnConfiguration getVpnConfiguration(Intent intent) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            return intent.getParcelableExtra(EXTRA_VPN_CONFIGURATION, VpnConfiguration.class);
        }
        return intent.getParcelableExtra(EXTRA_VPN_CONFIGURATION);
    }
